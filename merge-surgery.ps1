...
function Pause-Manual($msg) {
    Write-Host ""
    Write-Host "  ──── ACCIÓN MANUAL REQUERIDA ────" -ForegroundColor Magenta
...
# ============================================================
# FASE 0 — VERIFICACIÓN DE ESTADO INICIAL
# Copiar y pegar SOLO esta sección para empezar
# ============================================================
function Fase0-Verificar {
    Write-Phase 0 "VERIFICACIÓN DE ESTADO INICIAL"

    $branch = git branch --show-current
    if ($branch -ne $SOURCE_BRANCH) {
        Write-Fail "No estás en $SOURCE_BRANCH. Rama actual: $branch"
    ...
    if ($status) {
        ...
        if ($resp -eq "s") {
            ...
            Write-Warn "Continuando sin stash. Riesgo de pérdida de cambios."
        }
    } else {
...
function Fase1-FetchYDiffs {
    ...
    Pause-Manual "Confirma que leíste los 5 .patch y escribe ENTER para continuar."
}
...
function Fase5-ManualLogin {
    ...
    Write-Host ""
    Write-Host "  Reglas de resolución:" -ForegroundColor White
    ...
    if ($markers) {
        Write-Fail "Aún hay marcadores de conflicto en login/page.tsx. Resuélvelos primero."
    } else {
        git add -- "app/(auth)/login/page.tsx"
        Write-OK "login/page.tsx resuelto y añadido al index"
    }
}
...
function Fase6-ManualHackathon {
    ...
    if ($markers) {
        Write-Fail "Aún hay marcadores en hackathons/[id]/page.tsx. Resuélvelos primero."
    } else {
        git add -- "app/(dashboard)/dashboard/hackathons/[id]/page.tsx"
        Write-OK "hackathons/[id]/page.tsx resuelto y añadido al index"
    }
}
...
function Fase7-NonConflictFixes {
    ...
    Write-Info "Verificando topbar.tsx línea 61..."
    ...
    if ($hookBug) {
        ...
        Write-OK "topbar.tsx — hook correcto (o develop ya lo corrigió)"
    }
    ...
    Write-Host "  ¿develop añadió 'case superadmin' en getDashboardPathForRole?"
    $resp = Read-Host "  (s = sí, añadirlo / n = no, omitir)"
    if ($resp -eq "s") {
        code "lib/auth-helpers.ts"
        Pause-Manual "Añade el case superadmin y presiona ENTER."
    ...
    Write-Host "  ¿develop cambió el endpoint de approveChallenge?"
    $resp = Read-Host "  (s = sí, actualizar / n = no, omitir)"
    if ($resp -eq "s") {
...
function Fase8-BuildYVerificar {
    Write-Phase 8 "VERIFICACIÓN FINAL Y BUILD"
    ...
    if ($buildExitCode -ne 0) {
        Write-Fail "Build falló con código de salida $buildExitCode. Revisa build_output.txt antes de continuar."
        return
    }
...
function Fase9-SafetyGates {
    ...
    if (-not (Test-Path "build_output.txt")) {
        ...
        $buildExitFailed = $buildOutput -match "Build falló con código de salida|spawn EPERM|Build error occurred|ELIFECYCLE|npm ERR!"
...
function Recovery-Reset {
    ...
    if ($resp -eq "s") {
        ...
        Write-OK "Recovery completado. Estás de vuelta en $SOURCE_BRANCH"
    }
}

# ============================================================
# MENÚ DE EJECUCIÓN POR FASES
# ============================================================
Write-Host ""
Write-Host "╔══════════════════════════════════════════╗" -ForegroundColor Cyan
Write-Host "║      MERGE SURGERY — Ejecución por Fases ║" -ForegroundColor Cyan
Write-Host "╚══════════════════════════════════════════╝" -ForegroundColor Cyan
Write-Host ""
Write-Host "Funciones disponibles:" -ForegroundColor White
Write-Host "  Fase0-Verificar      — Verifica estado inicial"
Write-Host "  Fase1-FetchYDiffs    — Fetch y genera .patch para revisión"
Write-Host "  Fase2-CrearRama      — Crea rama merge-surgery-work"
Write-Host "  Fase3-IniciarMerge   — Inicia el merge (produce conflictos)"
Write-Host "  Fase4-TakeOurs       — Resuelve 14 archivos automáticamente"
...
Write-Host "NO ejecutar como script completo. Solo llamar cada función." -ForegroundColor Red
