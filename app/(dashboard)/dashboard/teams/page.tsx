...
const STATUS_CONFIG: Record<string, { labelKey: string; color: string }> = {
  forming: { labelKey: 'teams.status.forming', color: 'bg-unad-gold/10 text-unad-gold' },
  ready: { labelKey: 'teams.status.ready', color: 'bg-secondary/10 text-secondary' },
  active: { labelKey: 'teams.status.active', color: 'bg-emerald-500/10 text-emerald-500' },
  disbanded: { labelKey: 'teams.status.disbanded', color: 'bg-destructive/10 text-destructive' },
};
    ...
    <div className="space-y-6 animate-fade-in">
      ...
      {isLoading ? (
        ...
        <EmptyState
          ...
          title={t('teams.list.empty.title')}
          description={t('teams.list.empty.description')}
        >
          <div className="mb-2 flex items-center justify-center">
            <Link href="/dashboard/hackathons">
              <Button className="h-12 rounded-2xl bg-[#1c6d75] px-6 text-[15px] font-semibold text-white shadow-[0_14px_32px_rgba(28,109,117,0.28)] hover:bg-[#248f8b] hover:shadow-[0_18px_38px_rgba(36,143,139,0.34)]">
                {t('common.viewAll')} {t('nav.hackathons')}
